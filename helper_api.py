...
def _online_version() -> dict[str, str | int]:
    try:
        response = requests.get("https://raw.githubusercontent.com/WeikAiAim/sunone_aimbot/main/version", timeout=30)
...
def _reinstall_aimbot() -> tuple[bool, str]:
    ...
    ok, message = _download_file(
        "https://github.com/WeikAiAim/sunone_aimbot/archive/refs/heads/main.zip",
        archive,
    )
    if not ok:
